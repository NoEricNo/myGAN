...
def generate_samples(device, generator_r, generator_e, num_samples=1000, batch_size=100):
    ...
    # Calculate correct sparsity
    total_cells = df_matrix.size
    non_zero_cells = (df_matrix != 0).sum().sum()
    sparsity = 1 - (non_zero_cells / total_cells)

    # Print summary statistics
    print("\nRating Matrix Summary:")
    print(f"Shape: {df_matrix.shape}")
    print(f"Non-zero values: {non_zero_cells}")
...
def main():
    ...
    # Generate samples
    generate_samples(setup.device, setup.gan.generator_r, setup.gan.generator_e, 1000)

...
class GAN(nn.Module):
    def __init__(self, generator_r, generator_e, discriminator, device,
                 optimizer_g_r, optimizer_g_e, optimizer_d, logger):
    ...
    def train_epoch(self, data_loader, num_epochs, item_factors):
        ...
        for epoch in range(num_epochs):
            ...
            self.logger.info(
                ...
                f"GR Loss: {sum(epoch_g_r_losses) / len(epoch_g_r_losses):.4f}, "
                f"GE Loss: {sum(epoch_g_e_losses) / len(epoch_g_e_losses):.4f}" )

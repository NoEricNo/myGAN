...
class MovieLensDataset(Dataset):
    def __init__(self, ratings_file, chunk_size):
        self.rating_matrix, self.existence_matrix, self.num_movies, self.user_id_matrix, self.movie_id_matrix, self.num_users, self.num_movies = self.load_data(
            ratings_file)
    ...
    def load_data(self, ratings_file):
        ...
        rating_matrix = csr_matrix((rating_values, (user_indices, movie_indices)),
                                   shape=(num_users, num_movies)).toarray().astype(np.float32)
        ...
        return rating_matrix, existence_matrix, num_movies, user_id_matrix, movie_id_matrix, num_users, num_movies
    ...
    def __getitem__(self, idx):
        ...
        user_id_chunk = self.user_id_matrix[start_idx:end_idx].reshape(-1, 1)
        movie_id_chunk = self.movie_id_matrix[start_idx:end_idx].reshape(-1, 1)
        return data_chunk, existence_chunk, user_id_chunk, movie_id_chunk

    def shuffle_data(self):
        indices = np.arange(self.rating_matrix.shape[0])
        np.random.shuffle(indices)
        self.rating_matrix = self.rating_matrix[indices]
        self.existence_matrix = self.existence_matrix[indices]
